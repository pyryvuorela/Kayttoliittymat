...
public class RoomDatabase extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("create table " + TABLE_NAME + " (ID INTEGER PRIMARY KEY, ROOMNAME TEXT, LIGTH BOOLEAN, LOCK BOOLEAN, TEMP INTEGER)");
    }
    ...
    public boolean insertData(String roomname, Boolean ligth, Boolean lock, int temp){
    ...
    public boolean updateData(String roomname, Boolean ligth, Boolean lock, int temp){
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues contentValues = new ContentValues();
        contentValues.put(COL_2, ligth);
        contentValues.put(COL_3, lock);
